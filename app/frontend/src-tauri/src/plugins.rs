...
fn parse_skill_frontmatter(raw: &str) -> (Option<String>, Option<String>) {
    ...
    let rest = &content[3..];
    let end_idx = match rest.find("\n---") { Some(i) => i, None => return (None, None) };
    let fm = rest[..end_idx].trim();
    let mut name = None; let mut description = None;
    for line in fm.lines() {
        let line = line.trim();
        if line.is_empty() || line.starts_with('#') { continue; }
        if let Some((k, v)) = line.split_once(':') {
            let v = v.trim().trim_matches('"').trim_matches('\'');
            match k.trim() { "name" => name = Some(v.to_string()), "description" => description = Some(v.to_string()), _ => {} }
        }
    }
    (name, description)
}
...
fn discover_skills_in_dir(dir: &PathBuf) -> Vec<(String, String, String)> {
    ...
    if !dir.exists() || !dir.is_dir() { return results; }
    let entries = match std::fs::read_dir(dir) { Ok(e) => e, Err(_) => return results };
        ...
        if path.is_dir() {
            ...
            if skill_md.exists() && skill_md.is_file() {
                if let Ok(raw) = std::fs::read_to_string(&skill_md) {
                    ...
                    results.push((fm_name.unwrap_or(name_str), fm_desc.unwrap_or_default(), skill_md.to_string_lossy().to_string()));
                }
            } else { results.extend(discover_skills_in_dir(&path)); }
        }
    }
    results
}
...
fn read_skill_content(file_path: &str) -> SkillContent {
    ...
    let body = if raw.trim_start().starts_with("---") {
        ...
        if let Some(end_idx) = rest.find("\n---") { rest[end_idx + 4..].trim().to_string() } else { raw }
    } else { raw };
    SkillContent { name: name.unwrap_or_default(), description: description.unwrap_or_default(), body, file_path: file_path.to_string() }
}

fn skills_resource_dir() -> PathBuf {
    if let Ok(exe) = std::env::current_exe() {
        for ancestor in exe.ancestors() {
            let c = ancestor.join("resources").join("skills").join("skills-main").join("skills");
            if c.exists() { return c; }
        }
    }
    let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    let c = cwd.join("resources").join("skills").join("skills-main").join("skills");
    if c.exists() { return c; }
...
pub fn load_plugin_list(app: AppHandle) -> Result<Vec<PluginEntry>, String> {
    ...
    if !mgr.initialized {
        ...
        for (name, _desc, fp) in &discovered {
            mgr.content_cache.insert(name.clone(), read_skill_content(fp));
        }
        mgr.skill_entries = discovered.into_iter().enumerate().map(|(i, (name, desc, file_path))| PluginEntry {
            id: format!("skill-{}", i), name, description: desc,
            kind: "skill".to_string(), source: "imported".to_string(),
            installed: false, file_path, has_binary: None, bin_path: None, bin_dir: None,
        }).collect();

        // ── Plugins：内置注册表 ──
        mgr.plugin_entries = plugin_registry::builtin_plugins().into_iter().enumerate().map(|(i, def)| PluginEntry {
            id: format!("plugin-{}", i),
            name: def.name,
        ...
        if let Ok(db) = crate::storage::open_database(&app) {
            if let Ok(mut stmt) = db.prepare("SELECT name, kind FROM plugin_installs") {
                if let Ok(rows) = stmt.query_map([], |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))) {
                    for row in rows.flatten() { mgr.installed.insert(row); }
                }
            }
        }
    ...
    for entry in &mut mgr.skill_entries { entry.installed = installed_set.contains(&(entry.name.clone(), entry.kind.clone())); }
    ...
    for entry in &mut mgr.plugin_entries {
        ...
        if entry.file_path.is_empty() {
            let skill_path = plugin_registry::plugin_install_dir(&data_root, &entry.name).join("SKILL.md");
        ...
        if entry.has_binary == Some(true) && entry.bin_path.is_none() {
            if let Some(def) = plugin_registry::builtin_plugins().into_iter().find(|d| d.name == entry.name) {
                if let Some(bin_name) = def.binary_name {
                    let bin = plugin_registry::plugin_bin_dir(&data_root, &entry.name).join(&bin_name);
                    if bin.exists() {
...
fn install_skill(app: AppHandle, name: &str) -> Result<(), String> {
    ...
    if !exists { return Err(format!("未找到名为 '{}' 的 skill", name)); }

    let db = crate::storage::open_database(&app).map_err(|e| format!("无法打开数据库: {}", e))?;
    db.execute("INSERT OR REPLACE INTO plugin_installs (name, kind) VALUES (?1, 'skill')", params![name])
        .map_err(|e| format!("保存失败: {}", e))?;

    mgr.installed.insert((name.to_string(), "skill".to_string()));
    if !mgr.content_cache.contains_key(name) {
        let fp = mgr.skill_entries.iter().find(|e| e.name == name).map(|e| e.file_path.clone());
        if let Some(fp) = fp {
            mgr.content_cache.insert(name.to_string(), read_skill_content(&fp));
        }
    }
    Ok(())
}
...
fn uninstall_skill(app: AppHandle, name: &str) -> Result<(), String> {
    let mut mgr = PLUGINS_MANAGER.lock().map_err(|e| e.to_string())?;
    let db = crate::storage::open_database(&app).map_err(|e| format!("无法打开数据库: {}", e))?;
    db.execute("DELETE FROM plugin_installs WHERE name = ?1 AND kind = 'skill'", params![name])
        .map_err(|e| format!("删除失败: {}", e))?;
    mgr.installed.remove(&(name.to_string(), "skill".to_string()));
    Ok(())
}
...
fn install_plugin_kind(app: AppHandle, name: &str) -> Result<(), String> {
    // 1. 查注册表
    let def = plugin_registry::builtin_plugins().into_iter().find(|d| d.name == name)
    ...
    eprintln!("[plugins] {} SKILL.md 下载完成 → {}", name, skill_path.display());
    ...
    if !def.binaries.is_empty() {
        ...
        if let Some(bin_url) = def.binaries.get(platform) {
            ...
            eprintln!("[plugins] 正在下载 {} 二进制 ({}) 从 {}", name, platform, bin_url);
    ...
    // 6. 写 SQLite
    let db = crate::storage::open_database(&app).map_err(|e| format!("无法打开数据库: {}", e))?;
    db.execute("INSERT OR REPLACE INTO plugin_installs (name, kind) VALUES (?1, 'plugin')", params![name])
        .map_err(|e| format!("保存失败: {}", e))?;

    mgr.installed.insert((name.to_string(), "plugin".to_string()));
    Ok(())
}
...
fn uninstall_plugin_kind(app: AppHandle, name: &str) -> Result<(), String> {
    ...
    // 删除 SQLite 记录
    let db = crate::storage::open_database(&app).map_err(|e| format!("无法打开数据库: {}", e))?;
    db.execute("DELETE FROM plugin_installs WHERE name = ?1 AND kind = 'plugin'", params![name])
        .map_err(|e| format!("删除失败: {}", e))?;

    mgr.installed.remove(&(name.to_string(), "plugin".to_string()));
...
pub fn get_installed_skills_content() -> Vec<SkillContent> {
    let mgr = match PLUGINS_MANAGER.lock() { Ok(m) => m, Err(_) => return Vec::new() };
    mgr.installed.iter().filter_map(|(name, _kind)| mgr.content_cache.get(name).cloned()).collect()
}

/// 获取所有已安装的条目（skill + plugin）
pub fn get_installed_skill_entries() -> Vec<PluginEntry> {
    let mgr = match PLUGINS_MANAGER.lock() { Ok(m) => m, Err(_) => return Vec::new() };
    mgr.all_entries().into_iter().filter(|e| mgr.installed.contains(&(e.name.clone(), e.kind.clone()))).collect()
}
...
pub fn get_plugin_metadata(name: &str) -> Option<PluginEntry> {
    ...
    mgr.plugin_entries.iter()
        .find(|e| e.name == name && mgr.installed.contains(&(e.name.clone(), "plugin".to_string())))
        .cloned()
}
...
pub fn format_skills_for_prompt() -> String {
    ...
    let mut lines = vec![
        String::new(),
        "The following capabilities provide specialized instructions for specific tasks.".to_string(),
    ];
    ...
    if !plugins.is_empty() {
        ...
        for p in &plugins {
            ...
            lines.push(format!("    <description>{}</description>", escape_xml(&p.description)));
