...
pub fn enqueue_chat_message(request: EnqueueChatMessageRequest) -> Result<(), String> {
    let session_id = ai_runtime::require_text(&request.session_id, "session_id")?.to_string();
...
pub fn send_chat_message(
    ...
    let model_id = ai_runtime::require_text(&request.model_id, "模型")?.to_string();
    let providers = storage::load_api_configs(app.clone())?;
    let (provider, model) = ai_runtime::find_model(&providers, &model_id)?;
...
pub fn start_channel_agent_run(
    ...
    let providers = storage::load_api_configs(app.clone())?;
    let (provider, model) = ai_runtime::select_default_model(&providers)?;
    let mut settings = app_settings::load_settings(&app)?;
    let dialogue_root = session::agent_storage_root(&app)?;
    let prewrite_user_prompts = prompts.len() > 1;
    let channel_system_prompt = format!(
        "The current user is talking through {source_name}. You may use the same desktop tools available in the normal OtherOne client when the task requires it. Return only the plain text that can be sent back to that external user. Do not include internal tool logs, hidden reasoning, channel metadata, or desktop-only instructions in the reply. If you create or edit files, keep the text reply concise; the channel may deliver generated files separately. When the user asks you to send an existing local file, a desktop file, or a file generated by shell/PowerShell/REPL/Python/Office tooling, call send_file_to_weixin with the absolute file path. Do not say you cannot send files when this tool can mark the file for Weixin delivery."
    );
    ...
    let stream = tauri::async_runtime::block_on(async {
        ...
        let ai = build_ai_options(
            ...
            prompt_for_agent,
            reasoning_effort,
            AgentToolScope::WeixinFull,
        )?;
        Otherone::invoke_agent_stream(input, ai, None)
            .await
            .map_err(|error| error.to_string())
    })?;
...
fn build_ai_options(
    ...
    let api_key = ai_runtime::require_text(&provider.api_key, "API Key")?.to_string();
    let base_url = ai_runtime::require_text(&provider.base_url, "Base URL")?.to_string();
    ...
    let (tools_list, tools_map) = match tool_scope {
        ...
        AgentToolScope::WeixinFull => {
            tools::build_weixin_full_tools_for_session(app.clone(), session_id.to_string())
        }
    ...
    Ok(AiOptions {
        ...
        api_key,
        base_url,
        model: ai_runtime::require_text(&model.name, "模型名称")?.to_string(),
        ...
        tool_choice: ai_runtime::parse_tool_choice(&model.tool_choice),
        parallel_tool_calls: Some(model.parallel_tool_calls),
        stream: Some(true),
        other: ai_runtime::build_other_params(&model.extra_params, reasoning_effort)?,
    })
}
